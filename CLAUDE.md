...
- **`src/data/planData.vi.ts`** / **`planData.en.ts`** — the source of truth for all curriculum content (modules, resources, tech stack, quit criteria), one file per language, same shape and same `id`s. Pure data, zero UI logic. **Every item has a static `id` used as a localStorage primary key for completion state — never rename or reassign an existing `id`, and never let the two language files' id sets drift** (a dev-only check in `planData.ts` logs a console error if they do). `src/data/planData.ts` is a thin facade — `getSprintModules()` / `getMetaData()` / `getTechStackLayers()` / `getQuitCriteriaData()` — that picks a bundle by `state.lang`; call these instead of importing the arrays directly.
- **`src/i18n/`** — `strings.ts` (flat `UI_STRINGS` table per language), `index.ts` (`t(key, params)` / `plural(n, vi, enOne, enOther)`), `dom.ts` (`applyStaticTranslations()`, a `data-i18n` / `data-i18n-attr` sweep for the header markup in `index.html` that `renderAll()` doesn't reach — registered as a render listener in `main.ts`).
- **`src/state/storage.ts`** — singleton `AppState` (checked items, resourceFlags, activeTab, theme, lang, pomodoroSettings, pomodoroSessions), persisted to `localStorage` under `STORAGE_KEY`/`THEME_KEY`/`LANG_KEY` (`src/constants.ts`).
